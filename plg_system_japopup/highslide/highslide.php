	...
	if (!class_exists('highslideClass'))
	{
		class highslideClass extends JAPopupHelper
		{
			...
			function getHeaderLibrary()
			{
				...
				//JS
				$headtag[] = '<script type="text/javascript"> var expandDur = ' . $this->_pluginParams->get("group1-highslide-speed_in") . ' </script>';
				$headtag[] = '<script type="text/javascript"> var restoreDur = ' . $this->_pluginParams->get("group1-highslide-speed_out") . ' </script>';
			...
			function getContent($paras, $content)
			{
				...
				$arrData["outlineType"] = $this->_pluginParams->get("group1-highslide-outline") == '0'
					? 'null'
					: "'" . $this->_pluginParams->get("group1-highslide-outline") . "'";
				...
				$overlayOpacity = $this->_pluginParams->get("overlay_opacity") == '1'
					? 0.7
					: $this->_pluginParams->get("overlay_opacity");
					...
					case "image":
						{
							...
							$str .= '<a class="highslide" href="' . $arrData['href'] . '"
							onclick="return hs.expand(this, {dimmingOpacity:' . $arrData['overlayOpacity']
								. ', outlineType:' . $arrData['outlineType']
								. ' ' . $arrData['captionId'] . ',maxWidth:' . $arrData['frameWidth']
								. ', maxHeight:' . $arrData['frameHeight'] . '});" >'
								. $content . "</a>";
							break;
						}
