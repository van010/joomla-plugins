...
if ($direction == 'vertical')
	$direction='';
if((!empty($this->config->isComponent) && empty($this->config->isModule)) || (empty($this->config->isComponent) && !empty($this->config->isModule)) ) {
	HTMLHelper::script('components/com_eshop/assets/js/noconflict.js');
	HTMLHelper::script('components/com_eshop/assets/js/eshop.js');
	HTMLHelper::script('components/com_eshop/assets/colorbox/jquery.colorbox.js');
	HTMLHelper::stylesheet('components/com_eshop/assets/colorbox/colorbox.css');
	HTMLHelper::stylesheet('components/com_eshop/assets/css/labels.css');

	$shopper_groups = array(0);
	...
	if ($user->get('id'))
	{
		...
		$shopper_groups = array($customerGroupId);
	}
...
// 	{
// 			$productPriceArray = EshopHelper::getProductPriceArray($value->id, str_replace('$','',$value->frontend_price));
// 			if ($productPriceArray['salePrice']) {
// 				$discount[$value->id] = $currency->format($productPriceArray['salePrice']);
// 			}
// 			$productID[] = $value->id;
// 	}
// 	$quantities = $productID ? $this->getProductQuantity($productID) : array();
}
?>
...
<div class="jarow <?php echo $this->item['type'] ?> <?php echo $direction; ?> ja-megafilter-wrap clearfix">
	<?php if(!empty($this->config->fullpage) && !$hasModule): ?>
		<div data-mgfilter="eshop" class="<?php echo $direction ?> ja-mg-sidebar sidebar-main">
			<a href="javascript:void(0)" class="sidebar-toggle">
				<span class="filter-open">
					<i class="fa fa-filter"></i><?php echo Text::_('COM_JAMEGAFILTER_OPEN_FILTER'); ?>
				</span>
				<span class="filter-close">
					<i class="fa fa-close"></i><?php echo Text::_('COM_JAMEGAFILTER_CLOSE_FILTER'); ?>
				</span>
			</a>
...
<?php else: ?>
	<div data-mgfilter="eshop" class="<?php echo $direction; ?> ja-mg-sidebar sidebar-main">
		...
		<?php if(empty($this->config->isComponent)): ?>
			<a id="jamegafilter-search-btn" class="btn btn-default " href="javascript:void(0)"><?php echo Text::_('COM_JAMEGAFILTER_SEARCH') ?></a>
		<?php endif;?>
	</div>	
<?php endif; ?>
...
<?php endif; ?>	
var JABaseUrl = '<?php echo Uri::base(true); ?>';
...
function bindCallback() {
	setTimeout(function(){
		jQuery('.quantity-controls.quantity-plus, .quantity-controls.quantity-minus').off().unbind().on('click' ,function(){
			$val = jQuery(this).parents('div.addtocart-bar').find('input.quantity-input').val();
			$val = parseInt($val);
			if (jQuery(this).hasClass('quantity-plus'))
				$val++;
			else $val--;
			if ($val<1) $val=1;
			jQuery(this).parents('div.addtocart-bar').find('input.quantity-input').val($val);
		});
...
// 	if (discount[item.id] != undefined)
// 		item.frontend_price = discount[item.id];
// 	if (quantities[item.id] <= 0)
// 		item.is_salable = false;
	if ( item['shopper_groups'] && item['shopper_groups'].length !== 0 ) {
...
jQuery(document).ready(function() {
  var UBLNConfig = {};
  UBLNConfig.dataUrl = "<?php echo Uri::base(true).$this->config->json;  ?>";
